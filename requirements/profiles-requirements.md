...
3. A profile must reference exactly one source via `sourceId`.
4. Sources are managed independently through source APIs and linked to profiles.
...
1. As an analyst, I want to create a profile that links to a managed source so that I can reuse trusted URL and RSS definitions.
...
3. Saving a profile with unknown `sourceId` fails with `400` validation error.
4. Saving a profile with known `sourceId` succeeds and returns linked source context in profile responses.
